...
router.post(
    ...
    check('password', 'Password has to be 6 or more characters').isLength({min: 8}) ], 
    async (req, res)=> {
        ...
        try {
            ...
            user
                .create({name, email, password: encryptingPass, birthday, country, city})
                .then((doc)=>{
                    res.json({ success: true, user: doc.toJSON() });
                })
                .catch((err)=>{
                    res.json({ errors: [ { msg: 'Cannot register right now for some reason'} ]}) 
                })
        } catch (err) {
